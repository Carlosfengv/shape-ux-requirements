...
For a new or materially revised requirement, first use the reader-friendly alignment artifact produced by `$shape-requirement-baseline` in `ux-requirements.md`. It contains evidence, background, objectives, people, scenarios, terminology, scope, and complete candidate stories, but not final IA, detailed specifications, or ASCII UI. After explicit confirmation, extend the same document instead of composing a duplicate guide.
