...
2. confirmed story-to-functional-point map;
3. starting-surface decision table and ASCII page/detail/jump topology;
4. interaction-logic chain and material pattern-rationale table;
5. information architecture tree and IA node table;
6. ASCII navigation flow plus the applicable navigation behavior table when navigation spans steps or branches;
7. one ASCII task/decision flow plus its detailed table for every primary scenario;
8. ASCII swimlane or async sequence for every material cross-role handoff, wait, notification, or re-entry;
9. ASCII state flow for every material lifecycle;
10. mapping from stories through functions, pages, interactions, UI states, specifications, and acceptance criteria.
